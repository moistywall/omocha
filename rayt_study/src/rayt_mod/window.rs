...
pub fn draw_in_widow(backup_filename: &str, pixels: RgbImage) -> minifb::Result<()> {
    ...
    for (i, (_, _, pixels)) in buffer.iter_mut().zip(pixels.enumerate_pixels()) {
        *i = u32::from_be_bytes([0, pixels[0], pixels[1], pixels[2]]);
    }
        ...
        if window.is_key_pressed(Key::D, KeyRepeat::No) {
            if backup_buffer.is_none() {
                if let Ok(img) = image::open(backup_filename) {
                    ...
                    for (i, (_, _, pixels)) in buf.iter_mut().zip(backup_image.enumerate_pixels()) {
                        *i = u32::from_be_bytes([0, pixels[0], pixels[1], pixels[2]]);
                    }
